...
#loss function
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,eval_flag=False):
...
def train(env_name,train_steps = 200000,suffix="",all_loss=0,\
    ...
    for i in pbar:
        ...
        if (i+1) % eval_step ==0:
            #K loss
            with torch.no_grad():
                Reconloss, KLloss, Predloss = Klinear_loss(Ktest_data,net,mse_loss,u_dim,gamma,Nstate,all_loss=0)
                control_loss = Eig_loss(net) + Controlability_loss(net)
