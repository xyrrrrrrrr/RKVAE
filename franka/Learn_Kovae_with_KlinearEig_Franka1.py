...
def train(env_name,train_steps = 500000,suffix="",all_loss=0,\
    ...
    for i in pbar:
        ...
        # loss = Kloss
        pbar.set_postfix({"Franka: Total Loss": f"{loss.item():.6f}", "Pred Loss": f"{Predloss.item():.6f}", "Reconstruct Loss": f"{Reconloss:.6f}", "Control loss": f"{control_loss.item():.6f}", "KL Loss": f"{KLloss.item():.6f}", "Geom Loss": f"{Geomloss.item():.6f}"})
        optimizer.zero_grad()
        loss.backward()
        optimizer.step() 
